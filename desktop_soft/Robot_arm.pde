...
void setup() {
  ...
  runConnection =   new Runnable() {
    public void run() {
      if (!currentStatus.equals("connect")) {
         tryConnection();
      }
    }
  };
...
void draw() {
  ...
  // check connection
  if (port1 != null && currentStatus == "connect") {
    if (syncThread == null || !syncThread.isAlive()) {
      syncThread = new Thread(runSync);
      syncThread.start();
    }
  }
  ...
  if (b1.isPressed()) {
    b1Open = true;
    b2Open = false;
    if (currentStatus.equals("connect")) port1.write("b");
  }
  if (b2.isPressed()) {
    b1Open = false;
    b2Open = true;
    if (currentStatus.equals("connect")) port1.write("m");
  }
...
void keyPressed() {
  if (b1Open == true && currentStatus.equals("connect")) {
    if (key >= 'A' && key <= 'z') {
      port1.write(key);
      println(key);
    }
...
void mouseCheck() {
  if (b2Open == true && currentStatus.equals("connect")) {
    if (m1.isPressed()) {
      port1.write("q");
      println("Gripper+");
    }
